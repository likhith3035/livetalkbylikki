    ...
    <div className="flex min-h-full flex-1 flex-col bg-background selection:bg-primary/20">
      ...
      <main className="flex-1 px-5 py-8 pb-32 max-w-5xl mx-auto w-full">
        ...
        <div className="grid lg:grid-cols-[280px_1fr] gap-8 items-start">
          ...
          {/* Theme Mode Selector - Normal vs Liquid Glass */}
          <motion.section {...fadeUp} transition={{ delay: 0.12 }} className="space-y-3">
            <h2 className="text-[11px] font-extrabold text-primary/75 dark:text-primary/65 uppercase tracking-[0.22em] px-2.5 flex items-center gap-2">
              <Layers className="h-3.5 w-3.5 text-primary" /> Appearance Mode
            </h2>
            <div className="grid grid-cols-2 gap-3.5">
              ...
              <button
                type="button"
                onClick={() => {
                  updateSetting("liquidGlassEnabled", false);
                  toast({
                    title: "Classic Appearance",
                    description: "Reverted to solid default theme appearance."
                  });
                }}
                className={cn(
                  "flex flex-col items-center gap-3 p-4 rounded-3xl border text-center transition-all duration-300",
                  !settings.liquidGlassEnabled
                    ? "border-primary bg-primary/5 shadow-md shadow-primary/5"
                    : "border-border/50 bg-secondary/10 hover:border-primary/20 hover:bg-secondary/20"
                )}
              >
                <div className="h-11 w-11 rounded-2xl bg-secondary/80 flex items-center justify-center border border-border text-foreground">
                  <Sun className="h-5.5 w-5.5" />
                </div>
                <div>
                  <p className="text-xs font-bold uppercase tracking-wider text-foreground">Classic UI</p>
                  <p className="text-[9px] text-muted-foreground mt-0.5 leading-tight">Solid flat cards & default layouts</p>
                </div>
              </button>
              ...
              <button
                type="button"
                onClick={() => {
                  updateSetting("liquidGlassEnabled", true);
                  toast({
                    title: "🔮 Liquid Glass Active",
                    description: "Enjoy immersive frosted acrylic interfaces."
                  });
                }}
                className={cn(
                  "flex flex-col items-center gap-3 p-4 rounded-3xl border text-center transition-all duration-300 relative overflow-hidden",
                  settings.liquidGlassEnabled
                    ? "border-primary bg-primary/5 shadow-md shadow-primary/5"
                    : "border-border/50 bg-secondary/10 hover:border-primary/20 hover:bg-secondary/20"
                )}
              >
                ...
                <div className="h-11 w-11 rounded-2xl bg-primary/15 flex items-center justify-center border border-primary/25 text-primary">
                  <Layers className="h-5.5 w-5.5" />
                </div>
                <div>
                  <p className="text-xs font-bold uppercase tracking-wider text-foreground">Liquid Glass</p>
                  <p className="text-[9px] text-muted-foreground mt-0.5 leading-tight">Frosted overlays & fluid background blobs</p>
                </div>
              </button>
            </div>
          </motion.section>

          {/* Liquid Glass Studio Config (Only visible when active) */}
          <AnimatePresence>
            {settings.liquidGlassEnabled && (
              <motion.section 
                initial={{ opacity: 0, y: 15 }}
              ...
              >
                <h2 className="text-[11px] font-extrabold text-primary/75 dark:text-primary/65 uppercase tracking-[0.22em] px-2.5 flex items-center gap-2">
                  <Sliders className="h-3.5 w-3.5 text-primary" /> Liquid Glass Studio
                </h2>

                <div className="rounded-3xl border border-border/30 bg-card/20 backdrop-blur-md p-5 space-y-5 shadow-sm">
                  {/* Realtime Preview Card */}
                  <div className="space-y-1.5 lg:hidden">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/80 px-1">Live Studio Preview</label>
                    <GlassPreviewCard />
                  </div>

                  {/* Preset Choices */}
                  <div className="space-y-2">
                    <label className="text-xs font-semibold text-muted-foreground/80 px-1 flex items-center gap-1.5">
                      <Sparkles className="h-3 w-3 text-primary animate-pulse" /> Select Premium Preset
                    </label>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                      {(Object.keys(LIQUID_GLASS_PRESETS) as GlassPreset[]).map((key) => {
                        const preset = LIQUID_GLASS_PRESETS[key];
                        const isActive = settings.glassPreset === key;
                        return (
                          <button
                            key={key}
                            type="button"
                            onClick={() => {
                              updateSetting("glassPreset", key);
                              updateSetting("glassOpacity", preset.glassOpacity);
                              updateSetting("glassBlur", preset.glassBlur);
                              updateSetting("glassBorderOpacity", preset.glassBorderOpacity);
                              updateSetting("glassGlowIntensity", preset.glassGlowIntensity);
                              updateSetting("glassTintHSL", preset.glassTintHSL);
                              updateSetting("liquidBgSpeed", preset.liquidBgSpeed);
                              updateSetting("glassTextureIntensity", preset.glassTextureIntensity);
                              updateSetting("glassBorderWidth", preset.glassBorderWidth);
                              toast({
                                title: `✨ Preset Applied: ${preset.label}`,
                                description: `Visual styling updated to ${preset.label} settings.`
                              });
                            }}
                            className={cn(
                              "flex flex-col items-start gap-1.5 p-3 rounded-2xl border text-left transition-all duration-300",
                              isActive
                                ? "border-primary bg-primary/5 shadow-md shadow-primary/5"
                          ...
                          >
                            <div className="flex gap-1.5">
                              {preset.colors.slice(0, 3).map((col, idx) => (
                                <span key={idx} className="w-2.5 h-2.5 rounded-full border border-black/10 dark:border-white/10" style={{ backgroundColor: col }} />
                              ))}
                            </div>
                            <span className="text-[10px] font-bold text-foreground/85 leading-tight">{preset.label}</span>
                          </button>
                        );
                      })}
                    </div>
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-semibold text-muted-foreground/80 px-1 flex items-center gap-1.5">
                      <Palette className="h-3 w-3 text-primary" /> Glass Tint Color Suggestions
                    </label>
                    <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
                      {TINT_SUGGESTIONS.map((tint) => {
                        const isActive = settings.glassTintHSL === tint.hsl;
                        return (
                          <button
                            key={tint.label}
                            type="button"
                            onClick={() => {
                              updateSetting("glassPreset", "custom");
                              updateSetting("glassTintHSL", tint.hsl);
                            }}
                            className={cn(
                              "flex items-center gap-1.5 px-2.5 py-1.5 rounded-2xl border text-[10px] font-bold transition-all duration-300",
                              isActive
                                ? "border-primary bg-primary/10 text-foreground"
                                : "border-border/50 bg-secondary/15 hover:border-primary/20 hover:bg-secondary/25 text-muted-foreground hover:text-foreground"
                            )}
                          >
                            <span className="w-3.5 h-3.5 rounded-full border border-black/15 dark:border-white/15 shrink-0" style={{ backgroundColor: tint.color }} />
                            <span className="truncate">{tint.label.split(" ")[0]}</span>
                          </button>
                        );
                      })}
                    </div>
                  ...
                  {/* Fine Tuning Sliders */}
                  <div className="space-y-4.5 pt-3 border-t border-border/20">
                    <div className="flex items-center gap-1.5 text-xs font-bold text-muted-foreground/80 px-1">
                      <Sliders className="h-3 w-3 text-primary" /> Precision Controls
                    </div>

                    {/* Opacity */}
                    <div className="space-y-1.5">
                      <div className="flex justify-between text-[11px] font-bold">
                        <span className="text-muted-foreground">Glass Translucency (Opacity)</span>
                        <span className="font-mono text-foreground/80">{Math.round((1 - settings.glassOpacity) * 100)}%</span>
                      </div>
                      <input
                        type="range"
                        min="0.05"
                    ...
                    <div className="space-y-1.5">
                      <div className="flex justify-between text-[11px] font-bold">
                        <span className="text-muted-foreground">Backdrop Blur Depth</span>
                        <span className="font-mono text-foreground/80">{settings.glassBlur}px</span>
                      </div>
                      <input
                        type="range"
                        min="0"
                    ...
                    <div className="space-y-1.5">
                      <div className="flex justify-between text-[11px] font-bold">
                        <span className="text-muted-foreground">Border Transparency</span>
                        <span className="font-mono text-foreground/80">{Math.round(settings.glassBorderOpacity * 100)}%</span>
                      </div>
                      <input
                        type="range"
                        min="0.05"
                    ...
                    <div className="space-y-1.5">
                      <div className="flex justify-between text-[11px] font-bold">
                        <span className="text-muted-foreground">Glass Edge Glow</span>
                        <span className="font-mono text-foreground/80">{Math.round(settings.glassGlowIntensity * 100)}%</span>
                      </div>
                      <input
                        type="range"
                        min="0"
                        max="1"
                        step="0.05"
                        value={settings.glassGlowIntensity}
                        onChange={(e) => {
                          updateSetting("glassPreset", "custom");
                          updateSetting("glassGlowIntensity", parseFloat(e.target.value));
                        }}
                        className="w-full h-1 bg-secondary rounded-lg appearance-none cursor-pointer accent-primary focus:outline-none"
                      />
                    </div>
                    ...
                    <div className="space-y-1.5">
                      <div className="flex justify-between text-[11px] font-bold">
                        <span className="text-muted-foreground">Frosted Border Sheen Width</span>
                        <span className="font-mono text-foreground/80">{settings.glassBorderWidth}px</span>
                      </div>
                      <input
                        type="range"
                        min="0.5"
                        ...
                        onChange={(e) => {
                          updateSetting("glassPreset", "custom");
                          updateSetting("glassBorderWidth", parseFloat(e.target.value));
                        }}
                        className="w-full h-1 bg-secondary rounded-lg appearance-none cursor-pointer accent-primary focus:outline-none"
                      />
                    </div>

                    {/* Noise Texture Slider */}
                    <div className="space-y-1.5">
                      <div className="flex justify-between text-[11px] font-bold">
                        <span className="text-muted-foreground">Acrylic Noise Overlay Texture</span>
                        <span className="font-mono text-foreground/80">{Math.round(settings.glassTextureIntensity * 1000) / 10}%</span>
                      </div>
                      <input
                        type="range"
                        min="0"
                        max="0.15"
                        step="0.005"
                        value={settings.glassTextureIntensity}
                        onChange={(e) => {
                          updateSetting("glassPreset", "custom");
                          updateSetting("glassTextureIntensity", parseFloat(e.target.value));
                        }}
                        className="w-full h-1 bg-secondary rounded-lg appearance-none cursor-pointer accent-primary focus:outline-none"
                      />
                    </div>
                    ...
                    <div className="space-y-1.5">
                      <div className="flex justify-between text-[11px] font-bold">
                        <span className="text-muted-foreground">Liquid Flow Speed</span>
                        <span className="font-mono text-foreground/80">
                          {settings.liquidBgSpeed === 0 ? "Still" : settings.liquidBgSpeed === 10 ? "Maximum" : `${settings.liquidBgSpeed}/10`}
                        </span>
                      </div>
                      <input
                        type="range"
                        ...
                        max="10"
                        step="1"
                        value={settings.liquidBgSpeed}
                        onChange={(e) => {
                          updateSetting("glassPreset", "custom");
                          updateSetting("liquidBgSpeed", parseInt(e.target.value));
                        }}
                        className="w-full h-1 bg-secondary rounded-lg appearance-none cursor-pointer accent-primary focus:outline-none"
                      />
                    </div>
