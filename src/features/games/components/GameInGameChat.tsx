...
export const GameInGameChat: React.FC<GameInGameChatProps> = ({
  ...
  const handleSend = async (text: string) => {
    ...
    await sendGameChatMessage(roomCode, newMsg);
  };
  ...
  return (
    <>
      ...
      <div className="fixed bottom-14 left-2.5 sm:bottom-20 sm:left-4 z-40 select-none touch-manipulation">
        ...
        {!isOpen && (
          <motion.button
            ...
            className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-3.5 py-1.5 sm:py-2 rounded-2xl bg-card/95 backdrop-blur-xl border border-border shadow-xl hover:bg-muted text-foreground text-[11px] sm:text-xs font-bold transition-all relative cursor-pointer"
          >
            <MessageSquare className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-primary" />
            <span>Chat</span>
            {unreadCount > 0 && (
              <span className="w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-primary text-primary-foreground text-[9px] sm:text-[10px] font-black flex items-center justify-center animate-bounce shadow-md shadow-primary/30">
                {unreadCount}
              </span>
            )}
          </motion.button>
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="w-[calc(100vw-1.25rem)] max-w-sm sm:w-96 rounded-2xl sm:rounded-3xl bg-card/95 backdrop-blur-2xl border border-border/80 shadow-2xl overflow-hidden flex flex-col h-[380px] sm:h-[420px]"
            >
              {/* Header */}
              <div className="flex items-center justify-between px-3.5 py-2.5 sm:px-4 sm:py-3 border-b border-border/50 bg-muted/40">
              ...
              {/* Quick Chat Chips */}
              <div className="px-3 py-1.5 border-t border-border/40 bg-muted/20 flex items-center gap-1.5 overflow-x-auto no-scrollbar touch-pan-x">
                {QUICK_CHATS.map((chip, idx) => (
                  <button
                    key={idx}
                    onClick={() => handleSend(chip)}
                    className="px-2.5 py-1 rounded-full bg-muted hover:bg-primary/20 hover:text-primary text-[11px] font-medium shrink-0 border border-border/50 transition-all cursor-pointer active:scale-95"
                  >
                    {chip}
                  </button>
                ))}
              ...
              >
                ...
                <Button
                  ...
                  className="h-9 w-9 rounded-xl bg-primary text-primary-foreground shrink-0 shadow-md shadow-primary/20 cursor-pointer"
                >
