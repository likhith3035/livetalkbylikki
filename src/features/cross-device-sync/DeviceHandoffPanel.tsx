        ...
        {isConnected ? (
          <div className="space-y-3">
            ...
            <div className="rounded-xl bg-green-500/10 border border-green-500/20 px-3 py-2 flex items-center gap-2">
              <CheckCircle2 className="h-4 w-4 text-green-500 shrink-0 animate-bounce" />
            ...
            <div className="space-y-1.5">
              <p className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground px-1">Synced Devices</p>
                ...
                {participants.map((p) => (
                  <div
                    ...
                    className={cn(
                      ...
                      p.online ? "text-foreground" : "text-muted-foreground opacity-60"
                    )}
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      <Smartphone className={cn("h-3.5 w-3.5 shrink-0", p.online ? "text-primary animate-pulse" : "text-muted-foreground")} />
                      <span className="font-semibold truncate">
                        {p.name || (p.isHost ? "Primary Device" : "Secondary Device")}
                      </span>
                      {p.isHost && (
                        <span className="text-[8px] bg-primary/10 text-primary border border-primary/20 rounded px-1 font-bold shrink-0">Host</span>
                      )}
                    </div>
                    <div className="flex items-center gap-1.5 shrink-0">
                      <span className={cn("h-1.5 w-1.5 rounded-full", p.online ? "bg-green-500" : "bg-muted-foreground")} />
                      <span className="text-[9px] font-medium uppercase tracking-wider">{p.online ? "Online" : "Offline"}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="flex gap-2 pt-1 border-t border-border/20">
              <button
                onClick={copyLink}
                className={cn(
                  "flex-1 h-8 rounded-xl text-[10px] font-bold transition-all flex items-center justify-center gap-1 border",
                  settings.liquidGlassEnabled
                    ? "bg-white/5 border-white/10 text-foreground hover:bg-white/10"
                    : "bg-muted border-border/50 text-muted-foreground hover:bg-accent"
                )}
              >
                {copiedLink
              ...
              {onRefreshToken && (
                <button
                  ...
                  disabled={refreshing}
                  className={cn(
                    "flex-1 h-8 rounded-xl text-[10px] font-bold transition-colors disabled:opacity-40 border text-destructive hover:bg-destructive/10",
                    settings.liquidGlassEnabled
                      ? "bg-white/5 border-white/10 hover:border-destructive/30"
                      : "bg-muted border-border/50"
                  )}
                  title="Generate a new session code, disconnecting current devices"
                >
                  <RefreshCw
                    className={cn("h-3 w-3 mr-1 inline", refreshing && "animate-spin")}
                  />
                  New Code
                </button>
              )}
